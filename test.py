...
post_transformations_test = get_post_transformation(config, "test")
...
with torch.no_grad():
    num_sample=0
    for test_mini_batch in tqdm(test_loader, desc="Testing", total=min(len(test_loader), config["Test"].get("num_samples") or 9999999)):
        ...
        outputs, _ = model.inference(test_mini_batch, post_transformations_test, device=device, phase="test")
